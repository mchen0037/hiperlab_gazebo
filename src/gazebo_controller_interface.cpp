...
void GazeboControllerInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  cmd_motor_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_motor_speed_sub_topic_,
                                           ...
                                           this);
  std::cout << "********gazebo_controller_interface is subscribing to ~/" << model_->GetName() << command_motor_speed_sub_topic_ << std::endl;
  ...
  std::cout << "********gazebo_controller_interface is publishing to ~/" << model_->GetName() << motor_velocity_reference_pub_topic_ << std::endl;
}
