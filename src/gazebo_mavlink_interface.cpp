...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  std::cout << ">>>> gazebo_mavlink_interface SUBSCRIBES to: \n" <<
    "\t~/" + model_->GetName() + imu_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + lidar_sub_topic_ << "\n" <<
    // "\t~/" + model_->GetName() + opticalFlow_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + sonar_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + irlock_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + gps_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + groundtruth_sub_topic_ << "\n" <<
    "\t~/" + model_->GetName() + vision_sub_topic_ << "\n";


  // Publish gazebo's motor_speed message to /gazebo/default/iris/gazebo/command/motor_speed
  ...
  std::cout << ">>>> gazebo_mavlink_interface PUBLISHES to \n\t" <<  model_->GetName() + motor_velocity_reference_pub_topic_ << std::endl;
...
void GazeboMavlinkInterface::OnUpdate(const common::UpdateInfo&  /*_info*/) {
  ...
  if (received_first_referenc_) { // <><
    ...
    // std::cout << turning_velocities_msg.DebugString();
...
#if GAZEBO_MAJOR_VERSION >= 9 //mighty bookmark 1
