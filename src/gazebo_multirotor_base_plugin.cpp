...
void GazeboMultirotorBasePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  motor_pub_ = node_handle_->Advertise<mav_msgs::msgs::MotorSpeed>("~/" + model_->GetName() + motor_pub_topic_, 1);// who subscribes to this topic????

  std::cout << "gazebo_multirotor_base_plugin PUBLISHES to \n\t~/" << model_->GetName() + motor_pub_topic_ + "\n"; //<<<<<<<<<<<<<<<<, ~/iris/motors
...
void GazeboMultirotorBasePlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  MotorNumberToJointMap::iterator m; //iterate through every motor and joint and then get the velocity?
  ...
  // std::cout << msg.DebugString() << std::endl; // debug
  // motor_pub_->WaitForConnection();
  // Add time header
  motor_pub_->Publish(msg); //publish to ~/iris/motors
}
