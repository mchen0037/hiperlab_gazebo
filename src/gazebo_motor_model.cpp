...
void GazeboMotorModel::Publish() {
  turning_velocity_msg_.set_data(joint_->GetVelocity(0)); //get the x direction velocity of the rotor.
...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  */
  ...
  motor_velocity_pub_ = node_handle_->Advertise<std_msgs::msgs::Float>("~/" + model_->GetName() + motor_speed_pub_topic_, 1); //publish to ~/iris/motor_speed/0
  std::cout << "gazebo_motor_model SUBSCRIBES to\n\t" <<
    "~/" + model_->GetName() + command_sub_topic_ << "\n\t" <<
    motor_failure_sub_topic_ << "\n";

  std::cout << "gazebo_motor_model PUBLISHES to \n\t~/" + model_->GetName() + motor_speed_pub_topic_ + "\n";
...
void GazeboMotorModel::VelocityCallback(CommandMotorSpeedPtr &rot_velocities) {
  if(rot_velocities->motor_speed_size() < motor_number_) {
    ...
    //basically, if the motor_speed is bigger than the max, then just go the max.
    ref_motor_rot_vel_ =
      std::min(static_cast<double>(rot_velocities->motor_speed(motor_number_)),
      static_cast<double>(max_rot_velocity_)); //max_rot_velocity_ is 1100 (urf model for iris)
    }
}
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  double force = real_motor_velocity * real_motor_velocity * motor_constant_; //F = mv^2 / r
...
#else
  joint_->SetVelocity(0, turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_);
  if (std::abs(turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_ > .00001)) {}
    // std::cout << "Motor number " << motor_number_ << " is set to " << turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_ << std::endl;
#endif /* if 0 */
}
